...
const int buttonAnalogValues[] = { 992, 516, 341, 267, 217, 181 }; // Analog values corresponding to buttons
const int errorAnalogValue = 30;
...
ISR(SPI_STC_vect) { 
  ...
  if (data == Events::MasterRequestsInputFromSlave) {
    ...
    int buttonAnalogValue = analogRead(A4);
...
void debugButtonAnalogValues() {
  Serial.println(analogRead(A4)); // Read analog values for buttons
  delay(200);
}
